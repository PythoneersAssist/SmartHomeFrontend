...
export function FrontPage() {
  ...
  return (
    <div className={`${styles.gridPattern} min-h-screen text-slate-100`}>
      {/* Navbar */}
      <nav className={`${styles.navBar} fixed left-0 right-0 top-0 z-50`}>
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-400 to-lime-300">
          ...
          <div className="flex items-center gap-5">
            <a className="hidden text-sm font-medium text-slate-300 transition hover:text-emerald-300 md:inline" href="#features">Features</a>
            <a className="hidden text-sm font-medium text-slate-300 transition hover:text-emerald-300 md:inline" href="#how-it-works">How It Works</a>
            {isAuthenticated ? (
              <Link className="rounded-lg bg-gradient-to-r from-emerald-400 to-lime-300 px-4 py-2 text-sm font-bold text-slate-950 transition hover:-translate-y-0.5" to="/houses">
                Dashboard
              </Link>
            ) : (
              <div className="flex items-center gap-2">
                <Link className="rounded-lg px-3 py-2 text-sm font-semibold text-slate-200 transition hover:text-emerald-200" to="/login">
                  Sign In
                </Link>
                <Link className="rounded-lg bg-gradient-to-r from-emerald-400 to-lime-300 px-4 py-2 text-sm font-bold text-slate-950 transition hover:-translate-y-0.5" to="/register">
                  Get Started
                </Link>
              </div>
            )}
      ...
      <section className={`${styles.heroGlow} mx-auto grid w-full max-w-6xl gap-10 px-4 pb-16 pt-28 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:pt-32`}>
        <div>
          <p className="inline-flex rounded-full border border-white/20 bg-black/35 px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-emerald-200">
            IoT Smart Home Platform
          </p>
          <h1 className="mt-5 text-4xl font-black leading-[1.1] text-white md:text-[3.5rem]">
            Your Home,{' '}
            <span className="bg-gradient-to-r from-emerald-200 via-emerald-300 to-lime-200 bg-clip-text text-transparent">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {isAuthenticated ? (
              <Link
                className="rounded-xl bg-gradient-to-r from-emerald-400 to-lime-300 px-6 py-3.5 text-sm font-black text-slate-950 shadow-lg shadow-emerald-500/20 transition hover:-translate-y-0.5"
                to="/houses"
              >
                Open Dashboard
              </Link>
              ...
              <>
                ...
                <Link
                  className="rounded-xl border border-white/25 bg-black/35 px-6 py-3.5 text-sm font-bold text-slate-100 transition hover:-translate-y-0.5 hover:border-emerald-300/45"
                  to="/login"
                >
                  Sign In
                </Link>
          ...
          <div className="rounded-2xl border border-white/15 bg-black/45 p-4">
            <div className="mb-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-bold uppercase tracking-[0.15em] text-emerald-200">Dashboard Preview</p>
              </div>
            ...
            <div className="grid gap-2.5 sm:grid-cols-3">
              <article className="rounded-xl border border-white/12 bg-black/45 p-3">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Rooms</p>
                <p className="mt-1 text-xl font-black text-emerald-200">5</p>
              </article>
              <article className="rounded-xl border border-white/12 bg-black/45 p-3">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Devices</p>
                <p className="mt-1 text-xl font-black text-emerald-200">18</p>
              </article>
              <article className="rounded-xl border border-white/12 bg-black/45 p-3">
            ...
            <div className="mt-3 space-y-2">
              {['Living Room', 'Kitchen', 'Bedroom'].map((room) => (
                <div className="flex items-center justify-between rounded-lg border border-white/10 bg-black/35 px-3 py-2" key={room}>
                  <div className="flex items-center gap-2">
            ...
            <div className="mt-3 rounded-lg border border-white/10 bg-black/35 p-3">
              ...
              <div className="mt-2 flex gap-1.5">
                <span className="rounded-full bg-emerald-500/20 px-2 py-0.5 text-[9px] font-semibold text-emerald-200">Lights 7PM</span>
                <span className="rounded-full bg-white/10 px-2 py-0.5 text-[9px] font-semibold text-slate-200">AC 24°C</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Highlights strip */}
      <section className="border-y border-white/10 bg-black/35 py-10">
      ...
      <section className="mx-auto max-w-6xl px-4 py-16" id="features">
        <div className="text-center">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-300">Core Features</p>
      ...
      {/* How it works */}
      <section className="border-t border-white/10 py-16" id="how-it-works">
        <div className="mx-auto max-w-6xl px-4">
          <div className="text-center">
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-300">Getting Started</p>
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {[
              ...
              <div className="rounded-2xl border border-white/10 bg-black/35 p-6" key={item.step}>
      ...
      {/* CTA */}
      <section className={`${styles.ctaSection} border-t border-white/10 py-16`}>
        <div className="relative mx-auto max-w-3xl px-4 text-center">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            {isAuthenticated ? (
              <Link
                className="rounded-xl bg-gradient-to-r from-emerald-400 to-lime-300 px-8 py-3.5 text-sm font-black text-slate-950 shadow-lg shadow-emerald-500/20 transition hover:-translate-y-0.5"
                to="/houses"
              >
                Go to Dashboard
              </Link>
      ...
      <footer className="border-t border-white/10 py-8">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-4 md:flex-row">
          <div className="flex items-center gap-2">
            <div className="flex h-7 w-7 items-center justify-center rounded-md bg-gradient-to-br from-emerald-400 to-lime-300">
