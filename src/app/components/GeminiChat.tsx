...
export default function GeminiChat({ token }: { token: string }) {
  ...
  const [modalResolve, setModalResolve] = useState<((v: any) => void) | null>(
    null
  );
  ...
  async function loadHistory() {
    try {
      ...
      const parsed: ChatMessage[] = (data || []).map((d: any) => ({
        id: generateId(),
  ...
  function promptUserToPick(options: any[]) {
